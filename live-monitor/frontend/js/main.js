...
async function initAlertSettings() {
  ...
  document.getElementById("newAlertGroupBtn")?.addEventListener("click", () => {
...
function renderSelectedAlertGroup() {
  ...
  document.getElementById("alertGroupNameInput").value = group?.group_name || "";
  document.getElementById("alertGroupDescriptionInput").value = group?.description || "";
  document.getElementById("alertGroupEnabledInput").checked = group ? Boolean(group.enabled) : true;
...
function renderSelectedAlertChannel() {
  ...
  document.getElementById("alertChannelNameInput").value = channel?.channel_name || "";
  document.getElementById("alertChannelTypeSelect").value = channel?.channel_type || "email";
  document.getElementById("alertChannelEnabledInput").checked = channel ? Boolean(channel.enabled) : true;
  document.getElementById("alertEmailInput").value = channel?.alert_email || "";
  document.getElementById("alertMobileInput").value = formatMobilesForTextarea(channel?.alert_mobile || "");
  document.getElementById("smtpHostInput").value = channel?.smtp_host || "";
  document.getElementById("smtpPortInput").value = channel?.smtp_port || "";
  document.getElementById("smtpUserInput").value = channel?.smtp_user || "";
  document.getElementById("smtpPasswordInput").value = "";
  document.getElementById("smtpFromInput").value = channel?.smtp_from || "";
  document.getElementById("smtpTlsInput").checked = Boolean(channel?.smtp_use_tls);
  document.getElementById("smsApiUrlInput").value = channel?.webhook_url || channel?.sms_api_url || "";
  document.getElementById("smsApiTokenInput").value = "";
  document.getElementById("smsUsernameInput").value = channel?.sms_username || "";
  document.getElementById("smsPasswordInput").value = "";
  document.getElementById("smsPasswordIsMd5Input").checked = channel ? Boolean(channel.sms_password_is_md5) : true;
  document.getElementById("smsPasswordMd5Input").value = "";
  document.getElementById("smsRstypeInput").value = channel?.sms_rstype || "text";
  document.getElementById("smsExtCodeInput").value = channel?.sms_ext_code || "";
  syncChannelInputs();
}

function syncChannelInputs() {
  const channelType = document.getElementById("alertChannelTypeSelect")?.value || "email";
...
function buildAlertChannelPayload() {
  const channelType = document.getElementById("alertChannelTypeSelect").value;
  ...
  return {
    channel_name: document.getElementById("alertChannelNameInput").value.trim(),
    channel_type: channelType,
    alert_email: channelType === "email" ? document.getElementById("alertEmailInput").value.trim() || null : null,
    alert_mobile: channelType === "sms" ? document.getElementById("alertMobileInput").value.trim() || null : null,
    smtp_host: channelType === "email" ? document.getElementById("smtpHostInput").value.trim() || null : null,
    smtp_port: channelType === "email" && smtpPort ? Number(smtpPort) : null,
    smtp_user: channelType === "email" ? document.getElementById("smtpUserInput").value.trim() || null : null,
    smtp_password: channelType === "email" ? document.getElementById("smtpPasswordInput").value.trim() || null : null,
    smtp_from: channelType === "email" ? document.getElementById("smtpFromInput").value.trim() || null : null,
    smtp_use_tls: channelType === "email" ? document.getElementById("smtpTlsInput").checked : false,
    sms_api_url: channelType === "sms" ? apiUrl || null : null,
    sms_api_token: channelType === "sms" ? document.getElementById("smsApiTokenInput").value.trim() || null : null,
    sms_username: channelType === "sms" ? document.getElementById("smsUsernameInput").value.trim() || null : null,
    sms_password: channelType === "sms" ? document.getElementById("smsPasswordInput").value.trim() || null : null,
    sms_password_is_md5: channelType === "sms" ? document.getElementById("smsPasswordIsMd5Input").checked : true,
    sms_password_md5: channelType === "sms" ? document.getElementById("smsPasswordMd5Input").value.trim() || null : null,
    sms_rstype: channelType === "sms" ? document.getElementById("smsRstypeInput").value.trim() || "text" : "text",
    sms_ext_code: channelType === "sms" ? document.getElementById("smsExtCodeInput").value.trim() || null : null,
    webhook_url: ["webhook", "dingtalk"].includes(channelType) ? apiUrl || null : null,
    enabled: document.getElementById("alertChannelEnabledInput").checked,
  };
}
...
function renderAlertSettingsTable() {
  ...
  if (!alertSettingsState.services.length) {
    tbody.innerHTML = '<tr><td colspan="2" class="empty">暂无服务</td></tr>';
    return;
  }
